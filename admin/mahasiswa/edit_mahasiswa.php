...
<body>
    <div id="wrapper">
        ...
        <a class="navbar-brand" href="index.php">Adminstrator DigiCrafts</a>
    </div>
...
<div class="row">
    ...
    <div class="panel-body">
			...
			$sql = mysqli_query($koneksi, "SELECT * FROM mahasiswa WHERE username='$username'");if(mysqli_num_rows($sql) == 0){header('location:./../'.$_SESSION['username']);}else{$row = mysqli_fetch_assoc($sql);}
			if(isset($_POST['save'])){
	            
                $nama_lengkap     = aman($_POST['nama_lengkap']);
	            $password         = aman($_POST['password']);
	            $no_rekening      = aman($_POST['no_rekening']);
				$update = mysqli_query($koneksi, "UPDATE mahasiswa SET nama_lengkap='$nama_lengkap', password='$password', no_rekening='$no_rekening' WHERE username='$username'") or die(mysqli_error());
...
<div class="templatemo-content-container"><div class="templatemo-content-widget white-bg">
            <h2 class="margin-bottom-10">Edit Mahasiswa</h2><p>Edit Data Mahasiswa</p>
<form action="" class="templatemo-login-form" method="post" enctype="multipart/form-data">

                 <div class="row form-group">
                    <div class="col-lg-6 col-md-6 form-group"><label for="inputleadername">Username/NIM</label>
                    <input class="form-control" name="username" type="text" value="<?php echo $row['username']; ?>" placeholder="Username/NIM" disabled>
                    </div></div>
                ...
                <div class="row form-group"><div class="col-lg-6 col-md-6 form-group"><label for="inputleadername">Password</label>
                    <input type="text" name="password" class="form-control" value="<?php echo $row['password']; ?>" placeholder="*****">
				</div>
                <div class="col-lg-6 col-md-6 form-group"><label for="inputSquadName">Nomer Rekening</label>
                    <input type="text" name="no_rekening" class="form-control" value="<?php echo $row['no_rekening']; ?>" placeholder="20000101">
				</div></div>
                <br>
			<a href="mahasiswa.php" class="btn btn-warning"><span class="glyphicon glyphicon-arrow-left" aria-hidden="true"></span></a>
			<input type="submit" name="save" class="btn btn-primary" value="Simpan">
			<a href="lihat_mahasiswa.php?aksi=delete&username=<?php echo $row['username']; ?>" class="btn btn-danger" onclick="return confirm('Yakin?')"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Hapus Data</a>
