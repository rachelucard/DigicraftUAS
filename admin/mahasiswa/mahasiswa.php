...
<div class="row">
	<div class="col-lg-12"><div class="panel panel-default"><div class="panel-heading">Data Mahasiswa</div>
    <div class="panel-body">

<?php if(isset($_GET['aksi']) == 'delete'){$username = $_GET['username'];$cek = mysqli_query($koneksi, "SELECT * FROM mahasiswa WHERE username='$username'");
	  if(mysqli_num_rows($cek) == 0){echo '<div class="alert alert-info">Akun tidak ditemukan.</div>';}else{$delete = mysqli_query($koneksi, "DELETE FROM mahasiswa WHERE username='$username'");
...
<a href="tambah_mahasiswa.php"><button type="button" class="btn btn-default">Tambah Mahasiswa</button></a>
...
<?php
 if($urut){$sql = mysqli_query($koneksi, "SELECT * FROM  WHERE status='$urut' ORDER BY username ASC");}else{$sql = mysqli_query($koneksi, "SELECT * FROM mahasiswa ORDER BY username ASC");}
	  if(mysqli_num_rows($sql) == 0){echo '<tr><td colspan="8">Tidak ada data.</td></tr>';}else{$no = 1;while($row = mysqli_fetch_assoc($sql)){echo '
			<tr>
                ...
                <button type="button" class="btn btn-default btn-xs dropdown-toggle" data-toggle="dropdown">Actions<span class="caret"></span></button>
                        ...
                        <li><a href="lihat_mahasiswa.php?username='.$row['username'].'">Lihat Profil</a></li>
                        <li><a href="edit_mahasiswa.php?username='.$row['username'].'">Edit Akun</a></li>
                        <li class="divider"></li>
                        <li><a href="mahasiswa.php?aksi=delete&username='.$row['username'].'" >Hapus Akun</a></li>
                        </ul></div></td></tr>';$no++;}}?>     
