...
if(isset($_POST['update']))
{
 ...
 $sql1 = "UPDATE guest SET username='$username',password='$password', nama_lengkap='$nama_lengkap', email='$email', no_rekening='$no_rekening'WHERE username='$username'";

 $update1 = $dbconnect->query($sql1);
 if (! $update1) {
        echo "<script>alert('".$dbconnect->error."'); window.location.href = 'profil.php';</script>";
    } else {
        echo "<script>alert('Edit Iklan Berhasil'); window.location.href = 'profil.php';</script>";
    }
}
